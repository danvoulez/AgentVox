// Script para obter a chave correta do Supabase
const fs = require('fs');
const path = require('path');

console.log('=== INSTRUÇÕES PARA OBTER A CHAVE CORRETA DO SUPABASE ===\n');

console.log('Parece que a chave do Supabase que estamos usando está inválida.');
console.log('Vamos obter a chave correta diretamente do dashboard do Supabase.\n');

console.log('PASSO 1: Acesse o dashboard do Supabase');
console.log('- Abra o navegador e acesse: https://supabase.com/dashboard');
console.log('- Faça login na sua conta');
console.log('- Selecione o projeto "nwvxzlkhoobtotuixvpn"\n');

console.log('PASSO 2: Obtenha a chave anon key');
console.log('- No menu lateral, clique em "Project Settings"');
console.log('- Clique em "API"');
console.log('- Na seção "Project API keys", copie a chave "anon" ou "public"\n');

console.log('PASSO 3: Atualize o arquivo .env.local');
console.log('- Cole a chave copiada abaixo e salve este arquivo\n');

// Criar um arquivo para colar a chave
const keyFile = path.join(__dirname, 'supabase-key.txt');
fs.writeFileSync(keyFile, `# Cole a chave do Supabase abaixo:
NEXT_PUBLIC_SUPABASE_URL=https://nwvxzlkhoobtotuixvpn.supabase.co
NEXT_PUBLIC_SUPABASE_ANON_KEY=

# Após colar a chave, execute o comando:
# node update-env-with-key.js
`);

// Criar script para atualizar o arquivo .env.local
const updateScript = path.join(__dirname, 'update-env-with-key.js');
fs.writeFileSync(updateScript, `// Script para atualizar o arquivo .env.local com a chave do Supabase
const fs = require('fs');
const path = require('path');
...
console.log(`1. Edite o arquivo: ${keyFile}`);
console.log('2. Cole a chave do Supabase');
console.log('3. Execute: node update-env-with-key.js');
console.log('4. Verifique a conexão: node verify-updated-connection.js');
