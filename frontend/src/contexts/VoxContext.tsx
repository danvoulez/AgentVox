...
export const VoxProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    // Primeiro, definimos isLoading como true para evitar flashes
    setIsLoading(true);
    ...
    const handleAuthChange = async (session: any) => {
      try {
        ...
        // Sempre definimos isLoading como false no final
        setIsLoading(false);
      }
    };
  ...
  const loadUserData = async (userId: string) => {
    // Registrar início do carregamento para depuração
    console.log('Iniciando carregamento de dados do usuário:', userId);
    try {
      // Load memories
      ...
      const formattedMemories = memoriesData.map((memory: any) => ({
        id: memory.id,
  ...
  const deleteMemory = async (id: string): Promise<boolean> => {
    ...
    try {
      ...
      setMemories(prev => prev.filter((memory: any) => memory.id !== id));
  ...
  const retrieveRelevantMemories = async (query: string): Promise<VoxMemory[]> => {
    ...
    try {
      ...
      const response: Response = await fetch('/api/vox/memory/search', {
        method: 'POST',
        headers: {
  ...
  const signOut = async () => {
    try {
      setIsLoading(true); // Indicar que estamos processando o logout
