...
export const DecisionProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const removeDecision = (id: string) => {
    setDecisions(prev => prev.filter((decision: any) => decision.id !== id));
  ...
  const setActiveDecision = (id: string | null) => {
    ...
    const decision = decisions.find((d: any) => d.id === id) || null;
  ...
  const getActiveRules = () => {
    return rules.filter((rule: any) => rule.active)
  ...
  const getApplicableRules = (categories?: string[]) => {
    ...
    return getActiveRules().filter((rule: any) => 
  ...
  const getRecommendedOption = (decisionId: string): DecisionOption | null => {
    ...
    const compatibleOptions = decision.options.filter((option: any) => 
    ...
    const highPriorityRules = applicableRules.filter((rule: any) => rule.priority >= 8);
    ...
    return compatibleOptions.sort((a, b) => {
      ...
      if (highPriorityRules.length > 0) {
        ...
        const aMatchesHighPriorityRule = highPriorityRules.some((rule: any) => 
          (rule.timeOfDay === 'any' || rule.timeOfDay === theme) &&
          (a.categories?.includes(rule.category) || !rule.category)
        );
        
        const bMatchesHighPriorityRule = highPriorityRules.some((rule: any) => 
          (rule.timeOfDay === 'any' || rule.timeOfDay === theme) &&
  ...
  const getPendingDecisions = () => {
    return decisions.filter((d: any) => !d.completed);
  };
