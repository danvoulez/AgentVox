...
const DecisionRules: React.FC = () => {
  ...
  const handleDeleteRule = (id: string) => {
    setRules(prevRules => prevRules.filter((rule: any) => rule.id !== id));
    if (editingRuleId === id) {
      setEditingRuleId(null);
    }
  };
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleChange('priority', parseInt(e.target.value))}
                className="w-full p-2 rounded-md border"
                style={{ 
                  backgroundColor: 'rgba(var(--bg-color), 0.5)',
                  borderColor: 'rgba(var(--border-color), 0.5)'
            ...
            <div>
              ...
              <select
                ...
                onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleChange('timeOfDay', e.target.value)}
                className="w-full p-2 rounded-md border"
                style={{ 
                  backgroundColor: 'rgba(var(--bg-color), 0.5)',
                  borderColor: 'rgba(var(--border-color), 0.5)'
