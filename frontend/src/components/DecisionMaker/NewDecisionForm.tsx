...
const NewDecisionForm: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (options.some((option: any) => !option.title.trim())) {
      alert('Todas as opções precisam ter um título');
      return;
    }
    
    // Adicionar IDs às opções
    const optionsWithIds = options.map((option: any) => ({
      ...option,
      id: Math.random().toString(36).substring(2, 11),
      pros: option.pros.filter((pro: any) => pro.trim()),
      cons: option.cons.filter((con: any) => con.trim())
    }));
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input
                      type="text"
                      value={option.title}
                      onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleOptionChange(optionIndex, 'title', e.target.value)}
                      className="w-full p-2 rounded-md border"
                      style={{ 
                        backgroundColor: 'rgba(var(--bg-color), 0.5)',
                        borderColor: 'rgba(var(--border-color), 0.5)'
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleOptionChange(optionIndex, 'description', e.target.value)}
                      className="w-full p-2 rounded-md border"
                      style={{ 
                        backgroundColor: 'rgba(var(--bg-color), 0.5)',
                        borderColor: 'rgba(var(--border-color), 0.5)'
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <select
                        value={option.impact}
                        onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleOptionChange(optionIndex, 'impact', e.target.value)}
                        className="w-full p-2 rounded-md border"
                        style={{ 
                          backgroundColor: 'rgba(var(--bg-color), 0.5)',
                          borderColor: 'rgba(var(--border-color), 0.5)'
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleOptionChange(optionIndex, 'timeOfDay', e.target.value)}
                        className="w-full p-2 rounded-md border"
                        style={{ 
                          backgroundColor: 'rgba(var(--bg-color), 0.5)',
                          borderColor: 'rgba(var(--border-color), 0.5)'
                          ...
                          <input
                            ...
                            onChange={(e: React.ChangeEvent<HTMLInputElement>) => {
                              const newCategories = e.target.checked
                                ...
                                : (option.categories || []).filter((c: any) => c !== category);
                    ...
                    <div className="space-y-3 p-3 rounded-md" style={{ backgroundColor: 'rgba(var(--highlight-color), 0.05)' }}>
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleGradientChange(optionIndex, 'urgency', parseInt(e.target.value))}
                          className="w-full"
                        />
                      </div>
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleGradientChange(optionIndex, 'importance', parseInt(e.target.value))}
                          className="w-full"
                        />
                      </div>
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleGradientChange(optionIndex, 'confidence', parseInt(e.target.value))}
                          className="w-full"
                        />
                      </div>
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleGradientChange(optionIndex, 'emotionalImpact', parseInt(e.target.value))}
                          className="w-full"
                        />
                      </div>
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleGradientChange(optionIndex, 'longTermValue', parseInt(e.target.value))}
                          className="w-full"
                        />
                      </div>
                    </div>
                      ...
                      {option.pros.map((pro, proIndex) => (
                        <div key={proIndex} className="flex items-center mb-2">
                          <input
                            type="text"
                            value={pro}
                            onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleProChange(optionIndex, proIndex, e.target.value)}
                      ...
                      {option.cons.map((con, conIndex) => (
                        <div key={conIndex} className="flex items-center mb-2">
                          <input
                            type="text"
                            value={con}
                            onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleConChange(optionIndex, conIndex, e.target.value)}
                            className="flex-1 p-1 text-sm rounded-md border"
                            style={{ 
                              backgroundColor: 'rgba(var(--bg-color), 0.5)',
                              borderColor: 'rgba(var(--border-color), 0.5)'
