...
export default function KyteImport() {
  ...
  const importPeople = async () => {
    ...
    try {
      ...
      const mappedData = Array.isArray(data) 
        ? data.map(person => mapKytePerson(person))
        : [mapKytePerson(data)];
      
      setImportProgress(50);
      
      // Call the import API
      const response = await fetch('/api/kyte/import', {
        method: 'POST',
        headers: {
      ...
      toast({
        title: 'Import failed',
        description: error instanceof Error ? error instanceof Error ? error.message : "Unknown error" : 'An unknown error occurred',
        status: 'error',
        duration: 5000,
        isClosable: true,
      });
  ...
  const importProducts = async () => {
    ...
    try {
      ...
      // Map data to correct format if needed
      const mappedData = Array.isArray(data) 
        ? data.map(product => mapKyteProduct(product))
        : [mapKyteProduct(data)];
  ...
  const importSales = async () => {
    ...
    try {
      ...
      // Map data to correct format if needed
      const mappedData = Array.isArray(data) 
        ? data.map(sale => mapKyteSale(sale))
        : [mapKyteSale(data)];
  ...
  const importWhatsApp = async () => {
    ...
    try {
      ...
      // Call the import API
      const response = await fetch('/api/kyte/whatsapp-import', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
                  ...
                  <VStack spacing={4} align="stretch">
                    <FormControl>
                      ...
                      <Input
                        type="file"
                        accept=".json"
                        onChange={(e) => handleFileChange(e, setPeopleFile)}
                  ...
                  <VStack spacing={4} align="stretch">
                    <FormControl>
                      ...
                      <Input
                        type="file"
                        accept=".json"
                        onChange={(e) => handleFileChange(e, setProductsFile)}
                        disabled={isImporting}
                      />
                    </FormControl>
                  ...
                  <VStack spacing={4} align="stretch">
                    <FormControl>
                      ...
                      <Input
                        type="file"
                        accept=".json"
                        onChange={(e) => handleFileChange(e, setSalesFile)}
                        disabled={isImporting}
                      />
                    </FormControl>
                  ...
                  <VStack spacing={4} align="stretch">
                    <FormControl>
                      ...
                      <Input
                        type="file"
                        accept=".json"
                        onChange={(e) => handleFileChange(e, setWhatsappFile)}
                        disabled={isImporting}
                      />
                    </FormControl>
