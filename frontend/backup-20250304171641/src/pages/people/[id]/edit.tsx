    ...
    try {
      ...
      if (isNewPerson) {
        ...
        if (formData.functions.length > 0) {
          const functionRecords = formData.functions.map(functionType => ({
        ...
        const currentFunctionTypes = currentFunctions.map(f => f.function_type);
        const functionsToAdd = formData.functions.filter(f => !currentFunctionTypes.includes(f));
        
        // Add new functions
        if (functionsToAdd.length > 0) {
          const newFunctionRecords = functionsToAdd.map(functionType => ({
            person_id: id,
            function_type: functionType,
            is_active: true
          }));
