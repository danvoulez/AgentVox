...
const ArchetypeEvaluation: React.FC<ArchetypeEvaluationProps> = ({ option, showAll = false }) => {
  ...
  const evaluations = archetypesToShow.map(archetype => ({
    archetype,
      ...
      <div 
        ...
        style={{ 
          backgroundColor: highlight 
            ? `rgba(${archetype.color.replace('#', '').match(/.{1,2}/g)?.map(c => parseInt(c, 16)).join(', ') || '0, 0, 0'}, 0.1)` 
      ...
      {expanded && (
        <div className="mt-2 space-y-2 p-2">
          ...
          <div className="text-xs opacity-70 mt-2 p-2 rounded-md" style={{ backgroundColor: 'rgba(var(--highlight-color), 0.05)' }}>
            <p>Os conselheiros avaliam cada opção com base em diferentes perspectivas. {theme === 'dawn' ? 'Na madrugada' : theme === 'day' ? 'Durante o dia' : 'À noite'}, os arquétipos {currentTimeArchetypes.map(a => a.name).join(' e ')} são especialmente relevantes.</p>
          </div>
        </div>
      )}
    </div>
